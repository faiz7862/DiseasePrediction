...
class UI {
  ...
  getCartBtns() {
    ...
    addCartBtns.forEach((btn) => {
      ...
      if (isExist) {
        btn.textContent = 'You Are Connected ✌';
        btn.disabled = true;
      }

      btn.addEventListener('click', (e) => {
        //-> when btn clicked to add product to cart
        e.target.textContent = 'You Are Connected ✌';
  ...
  setCartValue(cart) {
    ...
    cartTotal.textContent = `Total Doctor Connected:  ${totalPrice.toFixed(2)}`;
  ...
  setUpApp() {
    ...
    cart.forEach((cartItem) => {
      ...
      if (addedCart) {
        addedCart.textContent = 'You Are Connected ✌';
  ...
  removeItem(id) {
    ...
    button.textContent = 'Connected to Doctors ';
